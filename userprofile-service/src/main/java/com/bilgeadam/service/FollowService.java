...
public class FollowService extends ServiceManager<Follow,String> {
    ...
    public Boolean createFollow(CreateFollowDto dto){
        ...
        if (authId.isEmpty()){
            throw new UserProfileManagerException(ErrorType.TOKEN_NOT_FOUND);// invalid token hatası ekle
        }
        Optional<UserProfile> userProfile = userProfileService.findOptionalByAuthId(authId.get()); // takip edecek kişi / takip eden
        Optional<UserProfile> followUser = userProfileService.findById(dto.getFollowId()); // takip edeceğim kişiler / takip edilen
